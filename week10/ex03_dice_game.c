...
// เพิ่มตัวนับ round ผ่าน pointer
void nextRound(int *round) {
    (*round)++;
    printf("\n--- Round %d ---\n", *round);
}

// รับ pointer ไปยัง score โดยตรง
...
// อ่านอย่างเดียว — const int* ป้องกันไม่ให้ฟังก์ชันนี้แก้ *score
void displayScore(const int *score) {
    printf("Current score: %d\n", *score);
}

// ถามผู้เล่นแล้วเขียนคำตอบลง *keepPlaying โดยตรง
...
int main() {
    ...
    // pointer เดียวที่ checkGuess และ displayScore ใช้ร่วมกันทุก round
    int *pScore = &score;
